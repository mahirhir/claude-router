...
- `tests/install-round-trip.ps1`, which runs `scripts/install.ps1` into a scratch directory and reads back everything the Quick start promises. The Quick start is a single command and nothing had ever run it: the Windows job parsed the installer and stopped there, so a broken installer would have kept every check green while failing every new user at step one. The gate asserts the target directory is absent beforehand, that every managed script arrives byte-identical, that `config.local.json` is created from `config.example.json`, that `-SkipPath` leaves the user PATH untouched, and that a second run overwrites a stale managed script while preserving an edited config - the two claims the README makes about re-running after a `git pull`. The expectation comes from `git ls-files`, not from the copy list inside the installer, so a filter bug there shows up here instead of being replayed. Deleting an installed script on purpose has to make the same readback fail *and* name the file, so a readback that has stopped being able to fail is distinguishable from today's pass.

### Changed

- `scripts/install.ps1` discovers what to copy instead of carrying a list of five file names: every `.ps1` and `.cmd` beside it is managed, and the installer itself is what stays behind. An empty result is an error rather than a silent no-op. A hand-written list is the same defect class as a CI step that names files - a script added later ships out of the clone but never reaches the installed directory, and nothing reports it.
...
- Both config readers now name the property and the config path they refused, and a missing property is a different message than an empty one, contributed by [@mahirhir](https://github.com/mahirhir) (#27). `Missing required config value: baseUrl` could not tell a reader whether the key was absent or blank, and it named neither the file it came from — on Windows a config that is a JSON array reported a missing `baseUrl`, which is a missing key in a file that has no keys at all. The two cases are now distinguishable at the message level on both sides, verified by mutation: removing the missing-property branch from `scripts/common.sh` reddens the missing-`mainModel` case while the blank-`authToken` case stays green. The same PR re-pointed the `tests/config-refusal-parity.sh` self-test at the wording that side uses today; its `sed` had targeted a string this change removes, so the reworded-tree case would have kept passing while testing nothing.

- `scripts/vscode-switch` resolves the default settings path per platform and takes `--insiders`, contributed by [@mahirhir](https://github.com/mahirhir) (#29): `~/Library/Application Support/Code/User/settings.json` on macOS, `$XDG_CONFIG_HOME/Code/User/settings.json` (defaulting to `~/.config`) elsewhere. Without the flag it always targets the stable edition, installed or not — with both editors present, silently writing to the one the user did not name is a misroute they have no way to see. Resolution reads `HOME` and `XDG_CONFIG_HOME`, and the not-found error prints the path it resolved, so the tests observe the resolution with no editor installed instead of passing an explicit path — the difference that makes them able to fail. Verified by mutation: collapsing the platform branch reddens three cases, where the earlier explicit-path tests survived it.
